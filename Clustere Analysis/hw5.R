...
#Principal
## nfactor = 2
fit5 = principal(logdata, nfactor=2, rotate="none")
fit5$loadings
round(var(fit5$scores), 4)
round(cor(fit5$x, fit5$scores), 4)

#NFACTOR =3
fit6 = principal(logdata, nfactor=3, rotate="none")
fit6$loadings
round(var(fit6$scores), 4)
round(cor(fit6$x, fit6$scores), 4)

#NFACTOR =4
fit7 = principal(logdata, nfactor=4, rotate="none")
fit7$loadings
#                 PC1   PC2   PC3   PC4
#SS loadings    4.058 1.981 0.948 0.823
#Proportion Var 0.451 0.220 0.105 0.091
#Cumulative Var 0.451 0.671 0.776 0.868
round(var(fit7$scores), 4)
round(cor(fit7$x, fit7$scores), 4)
cor(logdata, fit7$scores)
...
##DEFAULT-ROTATE
#NFACTOR =2
fit10 = principal(logdata, nfactor=2)
fit10$loadings
#                 RC1   RC2
#SS loadings    3.766 2.273
#Proportion Var 0.418 0.253
#Cumulative Var 0.418 0.671

#NFACTOR =3
fit9 = principal(logdata, nfactor=3)
fit9$loadings
#               RC1   RC2   RC3
#SS loadings    3.504 2.005 1.478
#Proportion Var 0.389 0.223 0.164
#Cumulative Var 0.389 0.612 0.776


#NFACTOR =4
fit8 = principal(logdata, nfactor=4)
fit8$loadings
round(var(fit8$scores), 4)
round(cor(fit8$x, fit8$scores), 4)
cor(logdata, fit8$scores)
##
#RC1   RC2   RC3   RC4
#SS loadings    2.907 1.965 1.477 1.461
#Proportion Var 0.323 0.218 0.164 0.162
#Cumulative Var 0.323 0.541 0.705 0.868
...
## PERCEPTUAL MAP
plot(fit2$x[,1:2], xlab="tasty", ylab="healthy",xlim=c(-6.0,6), ylim=c(-6.0,6), pch=16, col = data$mfr)
...
plot(fit8$x[,1:2], xlab="tasty", ylab="healthy",xlim=c(-6.0,6), ylim=c(-6.0,6), col=fit.th5$cluster+1, pch=16)
...
plot(fit9$x[,1:2], xlab="tasty", ylab="healthy",xlim=c(-6.0,6), ylim=c(-6.0,6), 
     col=fit.th4$cluster+1, pch=16)
...
plot(fit10$x[,1:2], xlab="tasty", ylab="healthy",xlim=c(-6.0,6), ylim=c(-6.0,6),
     col=fit.th3$cluster+1, pch=16)
...
cor(logdata)
sum(diag(cor(logdata)))
fit3 = eigen(cor(logdata))
fit3
scale(logdata) %*% fit$rotation

# simple example with princomp
fit4 = princomp(logdata, cor=T, scores=T)
fit4
fit4$loadings
fit4$scores
round(cor(fit4$x, fit4$scores), 4)
round(var(fit4$scores), 4)
